---
name: orquestrar-totum
description: >
  Kernel de orquestração do Totum OS. Ativa-se quando o usuário usa explicitamente
  "orquestrar-totum", "orquestra-totum", "@totum", "totum init", "totum start" ou "boot totum".
  Não disparar por menção isolada de "orquestrar" ou "organizar" sem outro sinal de complexidade
  junto. TAMBÉM dispara automaticamente quando o prompt tiver 150+ palavras, OU contiver 2+
  intenções distintas, OU contiver sinal de indecisão/múltiplas frentes mesmo em prompt curto
  (ex: "não sei por onde começar", "tenho três coisas pra resolver", "preciso de um plano pra
  isso", ou citar 3+ entregáveis distintos na mesma frase) — nesse caso analisa o prompt, mapeia
  as skills relevantes e sugere um plano de execução ANTES de agir. SEMPRE use esta skill ao
  iniciar sessões de trabalho Totum, pipelines com múltiplos entregáveis, ou quando o caminho de
  execução não estiver claro.
---

# orquestrar-totum — Kernel de Sessão Totum OS
...
Você **nunca executa a tarefa diretamente** — você delega. Exceção: tarefas triviais de 1 etapa
sem ambiguidade, onde criar um subagente seria overhead desnecessário.
...
- **Critério C (ambiguidade/indecisão):** prompt é curto (< 150 palavras, 1 intenção aparente),
  mas contém sinal explícito de indecisão ou múltiplas frentes — frases como "não sei por onde
  começar", "tenho várias coisas pra resolver", "preciso de um plano pra isso", ou menção a 3+
  entregáveis distintos na mesma frase sem detalhar cada um

> Intenções distintas = verbos de ação diferentes com objetos diferentes.
> "criar copy + fazer artes + montar fluxo N8N" = 3 intenções.
> "criar um copy longo com muitos exemplos" = 1 intenção (só tamanho conta).

> **Guarda contra falso positivo:** a palavra "organizar" ou "orquestrar" aparecendo sozinha,
> sem nenhum dos sinais acima, NÃO deve disparar esta skill. Exemplo que NÃO dispara: "vamos
> organizar essa ideia melhor". Exemplo que dispara pelo Critério C: "preciso organizar isso,
> não sei nem por onde começar, são várias frentes".

### Quando Critério A, B ou C for verdadeiro, execute:
...
Critério disparador: [Tamanho / Complexidade / Ambiguidade / combinação]
```
...
| `hermione` | pesquisa, metodologia, organização do conhecimento |
...
1. **Fase -1 é obrigatória** para prompts que atendam Critério A, B ou C — sem exceção.
...
### Exemplo 3 — Prompt curto com indecisão (Critério C, novo)

**Usuário:** `preciso organizar o lançamento do produto novo, tenho um monte de coisa pra fazer e nem sei por onde começar`
...
─────────────────
Palavras: 22
Intenções detectadas: não especificadas pelo usuário (sinal de indecisão explícito)
Critério disparador: Ambiguidade/Indecisão (Critério C)

Antes de sugerir skills, preciso mapear o escopo:
Quais frentes fazem parte desse lançamento? (copy, artes, automação, outro)
```
...
Ver seção "Regras de Operação" acima — a Fase -1 agora cobre três critérios (A, B, C) em vez de dois.

...
| [hds](frontend/hds/SKILL.md) *(legado)* | `frontend` | Motor original de Hero Design Specification — hoje incorporado ao `totum-blueprint`, que mantém "HDS" como alias | Compatibilidade com fluxos antigos que ainda chamam `hds` diretamente; para tudo novo, use `totum-blueprint` |
...
- `spec-feature` — mencionada como parte do Totum Kit Dev v2.2.0, mas não encontrada em nenhuma fonte disponível (nem `~/.claude/skills`, nem plugin, nem conta Cowork). Pendente de localização.
- `totum-script-vendas`, `totum-saida-completa`, `planejamento-estrategico`, `previa-hero`, `feed-perfeito` — existem como skills da conta mas não estavam no escopo desta missão; não foram avaliadas nem importadas.
